...
class BladeServiceProvider extends ServiceProvider
{
    ...
    protected function registerCaptcha(): void
    {
        ...
        Blade::directive('captcha', function ($lang) {
            ...
            if (config('grandstep.access.captcha.configs.options.hidden')) {
                $html .= new HtmlString('<style>.grecaptcha-badge{display:none;!important}</style>');
            }

            $html .= new HtmlString('
                <div class="g-recaptcha"
                    data-sitekey="'.config('grandstep.access.captcha.configs.site_key').'"
                    data-size="invisible"
                    data-callback="_submitForm"
                    data-badge="'.config('grandstep.access.captcha.configs.options.location').'">
                </div>');
